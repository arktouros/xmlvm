 ...
 * USA.
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:vm="http://xmlvm.org" xmlns:jvm="http://xmlvm.org/jvm" xmlns:dex="http://xmlvm.org/dex" version="2.0">
            ...
            <xsl:text>- (void) dealloc
{
            ...
            <xsl:if test="vm:method[@name='finalize' and                         not(vm:signature/vm:parameter) and                         vm:signature/vm:return[@type='void']]">
                <xsl:text>    [self finalize_</xsl:text>
...
@end
  ...
  _stack[_sp++].d = (double) _op1.i;</xsl:text>
    ...
    <xsl:text>    _op1.o = _stack[--_sp].o;
    _stack[_sp++].i = isObjectInstanceOf(_op1.o, "</xsl:text>
        <xsl:value-of select="vm:fixname(@type)"/>
        <xsl:text>");
</xsl:text>
    </xsl:template>
    ...
    <xsl:function name="vm:isObjectRef" as="xs:boolean">
        ...
        <xsl:value-of select="not($type='byte' or $type='short' or $type='int' or $type='float' or $type='long' or $type='double' or                             $type='char' or $type='boolean' or $type='void')"/>
    </xsl:function>
    ...
    <xsl:function name="vm:escapeString">
        ...
        <xsl:text>@"</xsl:text>
        <xsl:value-of select="replace(replace(replace(replace(replace(replace(replace($string,'\\011','\\t'),                            '\\012','\\n'),'\\015','\\r'),'\\014','\\f'),'\\010','\\b'),                            '\\042','\\&quot;'),'\\134','\\\\')"/>
        <xsl:text>"</xsl:text>
    </xsl:function>
