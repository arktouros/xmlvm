...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="winobjc">false</xsl:param>
            ...
            <xsl:text>- (void) dealloc
{
            ...
            <xsl:if test="vm:method[@name='finalize' and
                ...
                <xsl:text>    [self finalize_</xsl:text><xsl:value-of
                    select="vm:fixname(concat(@package, '.', @name))"/><xsl:text>__];
</xsl:text>
            </xsl:if>
...
<xsl:text>{
  ...
  <xsl:text>    _op1.i = _stack[--_sp].i;
    _op2.o = _stack[--_sp].o;
    _stack[_sp++].i = </xsl:text>
        <xsl:call-template name="get-array">
            <xsl:with-param name="opnum" select="2"/>
        </xsl:call-template>
        <xsl:text>.i[_op1.i];
</xsl:text>
    </xsl:template>
  ...
  <xsl:text>    _op1.f = _stack[--_sp].f;
    ...
    _op3.o = _stack[--_sp].o;
        ...
        <xsl:call-template name="get-array"/>
        <xsl:text>.f[_op2.i] = _op1.f;
</xsl:text>
    </xsl:template>

    <xsl:template name="get-array">
        <xsl:param name="opnum">3</xsl:param>
        <xsl:choose>
            <xsl:when test="$winobjc = 'true'">
                <xsl:text>[((XMLVMArray*) _op</xsl:text>
                <xsl:value-of select="$opnum"/>
                <xsl:text>.o) get_array]</xsl:text>
            </xsl:when>
            <xsl:otherwise>
                <xsl:text>((XMLVMArray*) _op3.o)->array</xsl:text>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
  ...
  <xsl:text>    _op1.o = _stack[--_sp].o;
    ...
    <xsl:function name="vm:useAutoReleasePool" as="xs:boolean">
        ...
        <xsl:variable name="flag"
                      select="$type/ancestor::vm:class/vm:annotations/vm:annotation[@type='org.xmlvm.iphone.XMLVMNoAutoReleasePool']"/>
  ...
  <xsl:text>    _op2.i = _stack[--_sp].i;
    _op1.l = _stack[--_sp].l;
    _stack[_sp++].l = ((JAVA_ULONG) _op1.l) &gt;&gt; _op2.i;</xsl:text>
    </xsl:template>
...
<xsl:text>{
   ...
   <xsl:text> = self;
    ...
    <xsl:template
            match="dex:invoke-direct|dex:invoke-direct-range|dex:invoke-virtual|dex:invoke-virtual-range|dex:invoke-interface|dex:invoke-interface-range">
        <xsl:variable name="returnTypedAccess">
            <xsl:call-template name="emitTypedAccess">
                <xsl:with-param name="type" select="dex:parameters/dex:return/@type"/>
            </xsl:call-template>
  ...
  <xsl:text>    [_ex release];
   ...
   _r</xsl:text><xsl:value-of select="@vx"/><xsl:text>.o = JAVA_NULL;
    ...
    <xsl:template
            match="dex:sget|dex:sget-wide|dex:sget-boolean|dex:sget-object|dex:sget-byte|dex:sget-short|dex:sget-char">
        <xsl:text>    _r</xsl:text>
        <xsl:value-of select="@vx"/>
        <xsl:call-template name="emitTypedAccess">
            <xsl:with-param name="type" select="@member-type"/>
    ...
    <xsl:template
            match="dex:sput|dex:sput-wide|dex:sput-boolean|dex:sput-char|dex:sput-byte|dex:sput-short|dex:sput-object">
        <xsl:text>    [</xsl:text>
        <xsl:value-of select="vm:fixname(@class-type)"/>
        <xsl:text> _PUT_</xsl:text>
        <xsl:value-of select="vm:fixname(@member-name)"/>
    ...
    <xsl:template
            match="dex:const-4[@kind='known-null']|dex:const-8[@kind='known-null']|dex:const-16[@kind='known-null']">
        <xsl:text>    _r</xsl:text>
        <xsl:value-of select="@vx"/>
        <xsl:text>.o = JAVA_NULL;
</xsl:text>
    ...
    <xsl:template
            match="dex:const|dex:const-4|dex:const-16|dex:const-wide|dex:const-wide-16|dex:const-wide-32|dex:const-high16|dex:const-wide-high16">
        <xsl:text>    _r</xsl:text>
        <xsl:value-of select="@vx"/>
        <xsl:call-template name="emitTypedAccess">
            <xsl:with-param name="type" select="@type"/>
    ...
    <xsl:template match="dex:fill-array-data">
        ...
        <xsl:value-of
                select="replace(replace(replace(replace(@vx-type, 'char', 'short'), 'byte', 'char'), 'long', 'JAVA_LONG'), 'boolean', 'char')"/>
        <xsl:text>){</xsl:text>
    ...
    <xsl:template match="dex:aget|dex:aget-wide|dex:aget-boolean|dex:aget-byte|dex:aget-char|dex:aget-short">
        ...
        <xsl:text> = </xsl:text>
        <xsl:call-template name="access-array">
            <xsl:with-param name="vy" select="@vy"/>
        </xsl:call-template>
    ...
    <xsl:template name="access-array">
        <xsl:param name="vy"/>
        <xsl:choose>
            <xsl:when test="$winobjc = 'true'">
    ...
    <xsl:template
            match="dex:move|dex:move-from16|dex:move-wide|dex:move-wide-from16|dex:move-object|dex:move-object-from16">
        <xsl:text>    _r</xsl:text>
        <xsl:value-of select="@vx"/>
        <xsl:text> = _r</xsl:text>
        <xsl:value-of select="@vy"/>
